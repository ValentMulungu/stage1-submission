...
def build_dim_customers(customers):
    ...
    dim_customers = (
        ...
        .withColumn(
            "income_band",
            F.col("income_band").cast("string") if "income_band" in customers.columns else F.lit(None).cast("string"),
        )
        .withColumn(
            "segment",
            F.col("segment").cast("string") if "segment" in customers.columns else F.lit(None).cast("string"),
        )
        .withColumn(
            "risk_score",
            F.col("risk_score").cast("int") if "risk_score" in customers.columns else F.lit(None).cast("int"),
        )
...
def build_dim_accounts(accounts, dim_customers):
    ...
    dim_accounts = (
        ...
        .withColumn(
            "product_tier",
            F.col("product_tier").cast("string") if "product_tier" in accounts.columns else F.lit(None).cast("string"),
        )
        .withColumn(
            "digital_channel",
            F.col("digital_channel").cast("string") if "digital_channel" in accounts.columns else F.lit(None).cast("string"),
        )
        .withColumn(
            "credit_limit",
        ...
        .withColumn(
            "last_activity_date",
            F.col("last_activity_date").cast(T.DateType()) if "last_activity_date" in accounts.columns else F.lit(None).cast(T.DateType()),
        )
        .select(
...
def count_orphaned_transactions(transactions, dim_accounts) -> int:
    account_lookup = dim_accounts.select("account_id").dropDuplicates(["account_id"])

    return int(
        transactions
        .join(account_lookup, on="account_id", how="left_anti")
        .count()
    )
...
def run_provisioning() -> None:
    ...
    orphaned_count = count_orphaned_transactions(transactions, dim_accounts)
    ...
    write_dq_report(
        "/data/output/gold/dq_report.json",
        counts={
            "ORPHANED_ACCOUNT": orphaned_count,
            "DUPLICATE_DEDUPED": 0,
            "TYPE_MISMATCH": 0,
            "DATE_FORMAT": 0,
            "CURRENCY_VARIANT": 0,
